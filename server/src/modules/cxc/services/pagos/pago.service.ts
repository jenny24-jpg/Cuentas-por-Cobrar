...
import * as aplicacionRepository from '../../repositories/pagos/aplicacionPago.repository';
...
export async function updatePago(id: number, raw: unknown): Promise<Pago> {
  ...
  const aplicado = await aplicacionRepository.sumAplicadoPorPago(id);
  if (aplicado > 0.005) {
...
export async function deletePago(id: number): Promise<void> {
  ...
  const aplicado = await aplicacionRepository.sumAplicadoPorPago(id);
  if (aplicado > 0.005) {
    throw new ConflictError('Un pago con aplicaciones no se elimina. Debe reversarse para conservar trazabilidad.');
  }
