...
export const AplicacionesPagoPage = () => {
  ...
  const { data, meta, isLoading, error, refetch } = usePaginatedList<AplicacionPago>('/cxc/aplicaciones-pago', { page, limit: 10, search });
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Aplicar un pago reduce el saldo del documento dentro de una transacción segura.</p>
        </div>
        <Button icon={Plus} onClick={() => setModal(true)}>Nueva Aplicación</Button>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200">
        <TextInput
          icon={Search}
          placeholder="Buscar por pago, documento o ID..."
          value={search}
      ...
      <DataTable
        ...
        columns={[
          ...
          { header: 'Pago', cell: ({ row }: any) => `Pago #${row.idPago}` },
          { header: 'Documento', cell: ({ row }: any) => `Documento #${row.idDocumento}` },
          { header: 'Fecha', accessorKey: 'fechaAplicacion', cell: ({ value }: any) => date(value) },
          { header: 'Monto aplicado', accessorKey: 'montoAplicado', cell: ({ value }: any) => money(value) },
          { header: 'Empleado', accessorKey: 'idEmpleado', cell: ({ value }: any) => value ? `#${value}` : '—' },
        ]}
        paginationProps={{
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Nueva Aplicación de Pago">
        <AplicacionPagoForm item={null} onCancel={() => setModal(false)} onSuccess={() => { setModal(false); refetch(); }} />
      </Modal>
    </div>
  );
};
